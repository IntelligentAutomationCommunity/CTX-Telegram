...
### Inputs
- **i_telegram-apr-url**: (Text) The URL of the Telegram Bot API server (Optional - if not provided then the [CTX-Configuration-Store](https://github.com/CortexIntelligentAutomation/CTX-Configuration-Store) area *Telegram* parameter *URL* is used)
- **i_bot-token**: (Text) The token assigned by the [Telegram BotFather](https://core.telegram.org/bots#creating-a-new-bot) to the bot which will be posting to the Telegram chat. (Optional - if not provided then the [CTX-Configuration-Store](https://github.com/CortexIntelligentAutomation/CTX-Configuration-Store) area *Telegram* parameter *token* is used)
...
- **i_parse_mode**: (Text) The [Telegram formatting option](https://core.telegram.org/bots/api#formatting-options) for the message (Optional)
